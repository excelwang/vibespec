...
11. If the published batch contains defects, release bounded fix work without waiting for later classes to finish, but keep shared-state ownership on triage until the full class cycle is complete.
...
15. End the bounded iteration with updated shared state; do not improvise a second coordination protocol inside this prompt.
```
...
11. Triage may still be classifying later defect classes while fix starts on earlier released work, but Fix must not mutate shared coordination state until the baton actually moves.
...
16. End the bounded worker iteration by returning control to the coordinator after the required publication or wait outcome; do not invent cross-role work.
```
